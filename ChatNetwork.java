...
class Global {
	...
	// Server Response should have src-nick signed using Server-private key to prevent man in the middle attacks. Protocol modification required.
	// friendValid = code|E(aes-key){dest-name:E(root-private){b64e{src-nick}|dest-nonce}}|route
	// friendProve = code|b64e{src-nick}:b64e{dest-nick}|E(session-key){src-nonce:E(root-private){b64e{src-nick}|dest-nonce}}}
	...
	public static volatile ArrayList<ChatResponse> chatResponse = new ArrayList<ChatResponse>();
	...
	public static synchronized void process(TcpClient that, String command, int LinkId){
		if(command.startsWith(Global.serverTimestamp)){
			...
			if(temp2.length>0){
				...
				if(command.startsWith(Global.serverHelloAccept) && Global.debugLevel>=2) System.out.println("Logged in as "+Global.username+".");
				if(command.startsWith(Global.serverRegisterAccept) && Global.debugLevel>=2) System.out.println("Registered as "+Global.username+".");
				...
				try {
					...
					// Marker : Do something with the list of friends.
					}
					...
					if(name2.equals(Global.username)){
						...
						for(int i=0;i<Global.chats.size();++i)
							if(Global.chats.get(i).name.equals(name1)){
								...
								Global.chatResponse.add(new ChatResponse(name1,Global.localTime));
								break;
								}
						}
...
class ChatSession {
	...
	public void send(String data){
		data = "1"+Global.delimiter3+this.sequence+Global.delimiter2+data;
		++this.sequence;
	...
	public void recv(String data){
		...
		if(data.startsWith("0")){
			for(int i=0;i<this.messages.size();++i)
				if(this.messages.get(i).startsWith("1"+Global.delimiter3+sequence+Global.delimiter2)){
		...
		if(data.startsWith("1")){
			this.send_actual("0"+Global.delimiter3+sequence+Global.delimiter2);
			if(Global.debugLevel>=1) System.out.println("Message from "+this.name+" ["+sequence+"] : "+data.substring(data.indexOf(Global.delimiter2)+1));
			// Marker : Do something with the newly received message
			}
		}
	}
...
class ChatResponse{ public String name; public int time; ChatResponse(String n,int t){ this.name=n; this.time=t; } }
...
class StepAction implements Runnable {
	...
	public void run(){
		while(this.active){ Global.sleep(1000); ++Global.localTime;
			...
			// After a while, clear old chat requests and responses
			for(int i=0; i<Global.chatLocateRequest.size();++i)
				if(Global.chatLocateRequest.get(i).time+Global.sessionTime<Global.localTime)
					{ Global.chatLocateRequest.remove(i); --i; }
			for(int i=0; i<Global.chatResponse.size();++i)
				if(Global.chatResponse.get(i).time+Global.sessionTime<Global.localTime)
					{ Global.chatResponse.remove(i); --i; }
...
class ChatNetwork implements Runnable {
	...
	public static void main(String args[]){
		if(args.length<2){ System.out.println("Insufficient number of command line arguments."); return; }
	...
	public void run(){
		...
		try {
			...
			InetAddress broadcast = networkInterface.getInterfaceAddresses().get(0).getAddress(); // debug ... broadcast was returning 0.0.0.0 which caused udp send error
			String ip1 = localHost.getHostAddress(); if(ip1.indexOf("/")!=-1) ip1 = ip1.substring(ip1.indexOf("/")+1);
			...
			//System.out.println(networkInterface.getInterfaceAddresses().get(0).getBroadcast());
			}
		catch(Exception e){ e.printStackTrace(); }
...
To start all network operations:
	ChatNetwork cn = new ChatNetwork(username,password);
To add a user to your friends list:
	Global.friend(friendname,true);
To remove a user from your friends list:
	Global.friend(friendname,false);
The list of all friends can be found at:
	ArrayList<String> Global.friends
To send a message to a friend:
	Global.chat(friendname,message);
To shut down all network operations:
	Global.close();
	
What to do when a FriendList Update arrives?
What to do when a message from a Friend arrives?
	Markers inserted in code.
